...
if [ "$FROM_JENKINS" = "true" ]; then
  ...
  oc annotate namespace ${GUID}-jenkins    openshift.io/requester=${USER} --overwrite=true
  oc annotate namespace ${GUID}-tasks-dev  openshift.io/requester=${USER} --overwrite=true
  oc annotate namespace ${GUID}-tasks-prod openshift.io/requester=${USER} --overwrite=true
fi
